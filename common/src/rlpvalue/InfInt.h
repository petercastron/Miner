...
//in   "123467896149831319643031646031646316461303"
//out  bytes
inline static std::string encode_bignum(const InfInt& n)
{
	std::string rs = "";

	if (n != 0) {
		rs.assign(encode_bignum(n / 256));
...
//in   123467896149831319643031646031646316461303
//out  bytes
inline static std::string encode_bignum_str(const std::string& str_num)
{
	InfInt n(str_num);
	return encode_bignum(n);
}

//in   bytes
//out  123467896149831319643031646031646316461303
inline static InfInt decode_bignum(const u_char *raw, size_t len)
{
  if (len == 0) return 0;
  else if (len == 1) return *raw;
  else return (decode_bignum(raw, len - 1) * 256) + (raw[len - 1]);
}

inline static InfInt decode_bignum(const char *raw, size_t len)
{
  return decode_bignum((const u_char *)raw, len);
}

inline static InfInt decode_bignum(const std::string& str_bytes)
{
  return decode_bignum(str_bytes.c_str(), str_bytes.size());
}

//in   bytes
//out  "123467896149831319643031646031646316461303"
inline static std::string decode_bignum_str(const u_char *raw, size_t len)
{
  return decode_bignum(raw, len).toString();
}

inline static std::string decode_bignum_str(const char *raw, size_t len)
{
  return decode_bignum((const u_char *)raw, len).toString();
}

inline static std::string decode_bignum_str(const std::string& str_bytes)
{
   return decode_bignum(str_bytes.c_str(), str_bytes.size()).toString();
}
#endif
